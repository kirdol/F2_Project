...
## Replace "Not Available" with NA, then 0 (Production)
...
j'arrive pas pour l'instant, problèmes de conversion de character à numeric

## Plot Temperature

A refaire, ç<a ne marche pas non plus

```{r}
temperature <- temperature %>% 
  mutate(Date = make_date(Year, Month, 1)) %>% 
  select(-Year, -Month) %>% 
  filter(year(Date) == 1973) #BRUH CA MARCHE PAS

ggplot(temperature, aes(x = Month, y = Total_Primary_Energy_Production)) + 
  geom_line() + 
  labs(title = "Time Series Plot", x = "Date", y = "Value")
```
